...
func (b *Backend) judgeOne(ctx context.Context, j backend.Judgement) (backend.Result, error) {
	...
	value, verr := coerceResult(constraint, completion.Content)
	if verr != nil {
...
func (b *Backend) buildRequest(j backend.Judgement, constraint schema.Constraint) completionRequest {
	...
	return completionRequest{
		Model:       model,
		Prompt:      buildPrompt(j, constraint),
		NPredict:    npredict,
		Temperature: 0,
...
// coerceResult parses the daemon's textual content into the constraint's return
// type and validates it against the constraint (including classify enum
// membership). A parse, type, or enum mismatch yields an error which the caller
// records as a per-item error, enforcing schema invariance at the boundary.
