...
if ($workflow -notmatch 'finalize-release:[\s\S]*?name: Check out release tag[\s\S]*?name: Download all promoted draft assets') {
    throw 'release finalization must check out the release tag before downloading and validating draft assets'
}
...
if (-not $packageSmoke.Contains('[string]$StandardInput') -or -not $packageSmoke.Contains('$process.StandardInput.Write($StandardInput)')) { throw 'package manager smoke must preserve fixture stdin without using the automatic $input variable' }
